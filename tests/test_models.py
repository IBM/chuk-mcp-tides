...
def test_station_detail_response_to_text():
    r = StationDetailResponse(
        station_id="123", name="Test Station", provider="noaa",
        lat=40.0, lon=-74.0, message="Station detail",
...
def test_station_detail_minimal():
    r = StationDetailResponse(
        station_id="123", name="Minimal", provider="ea",
        lat=51.0, lon=-0.1, message="detail",
    )
    text = r.to_text()
...
def test_prediction_response_to_text():
    r = PredictionResponse(
        station_id="123", station_name="Test", provider="noaa",
        datum="MLLW", units="metric", start_date="2024-01-01",
        end_date="2024-01-07", interval="hilo", event_count=2,
        predictions=[
...
def test_prediction_response_truncation():
    ...
    r = PredictionResponse(
        station_id="123", station_name="Test", provider="noaa",
        datum="MLLW", units="metric", start_date="2024-01-01",
        end_date="2024-01-30", interval="hourly", event_count=29,
        predictions=preds, message="29 predictions",
    )
    text = r.to_text()
...
def test_local_prediction_response_to_text():
    r = LocalPredictionResponse(
        constituent_count=37, start_date="2024-01-01",
        end_date="2024-01-07", interval_minutes=60,
        event_count=168, predictions=[],
...
def test_observation_response_to_text():
    r = ObservationResponse(
        station_id="123", station_name="Test", provider="noaa",
        datum="MLLW", units="metric", product="water_level",
        reading_count=1,
...
def test_latest_reading_response_to_text():
    r = LatestReadingResponse(
        station_id="123", station_name="Test",
        datetime="2024-01-01T12:00", value=0.7,
        datum="MLLW", units="metric", tide_state="rising",
...
def test_threshold_exceedance_response_to_text():
    r = ThresholdExceedanceResponse(
        station_id="123", threshold=1.5, datum="MLLW", source="predictions",
        total_exceedances=30, total_hours_above=6.0,
...
def test_flood_projection_response_to_text():
    r = FloodProjectionResponse(
        station_id="123", station_name="Test", threshold=1.5, datum="MLLW",
        baseline_year=2024, baseline_exceedances=10,
        projections=[
            FloodProjection(
                year=2050, scenario="low", slr_mm=150,
                projected_exceedances=20, projected_hours=4.0, delta_from_baseline=10,
            ),
        ],
...
def test_harmonic_analysis_response_to_text():
    r = HarmonicAnalysisResponse(
        station_id="123", observation_days=60, constituent_count=2,
        constituents=[
            ConstituentResult(name="M2", amplitude=0.5, phase=123.4, snr=50.0),
            ConstituentResult(name="S2", amplitude=0.2, phase=45.0),
        ],
        mean_level=0.1, form_number=0.15, tidal_type="semidiurnal",
        residual_std=0.05, stored=True, message="Harmonic analysis",
    )
    text = r.to_text()
    assert "M2" in text
    assert "semidiurnal" in text
...
def test_residual_response_to_text():
    r = ResidualResponse(
        station_id="123", reading_count=100,
        ...
        mean_residual=0.01, std_residual=0.1,
        surge_events=[], residuals=[], message="Residual analysis",
    )
    text = r.to_text()
...
def test_sea_level_trend_response_to_text():
    r = SeaLevelTrendResponse(
        station_id="123", station_name="Test",
        trend_mm_per_year=3.5, trend_uncertainty=0.2,
        first_year=1900, last_year=2024, record_length_years=125,
        data_source="NOAA", message="Sea level trend",
    )
    text = r.to_text()
    assert "+3.50" in text
    assert "1900" in text
...
def test_extreme_levels_response_to_text():
    r = ExtremeLevelsResponse(
        ...
        datum="MLLW", message="Extreme levels",
    )
    text = r.to_text()
...
def test_flood_outlook_response_to_text():
    r = FloodOutlookResponse(
        station_id="123", product="htf_annual", flood_threshold="minor",
        flood_level_m=1.04,
...
def test_flooding_calendar_response_to_text():
    r = FloodingCalendarResponse(
        station_id="123", year=2024, threshold=1.5, slr_offset_mm=50,
        datum="MLLW", total_flood_days=10, total_flood_hours=20.0,
        monthly_summary=[
...
def test_status_response():
    r = StatusResponse(
        server="chuk-mcp-tides", version="0.1.0", storage_provider="memory",
        providers=[ProviderStatus(name="NOAA", available=True, station_count=3000, auth_configured=True)],
        harmonic_engine="utide", stored_constituents=0,
    )
    text = r.to_text()
    assert "NOAA" in text
...
def test_capabilities_response_to_text():
    r = CapabilitiesResponse(
        server="chuk-mcp-tides", version="0.1.0",
        providers=[
            ProviderInfo(name="NOAA CO-OPS", short_name="noaa", coverage="US",
                         auth_required=False, station_count=3000),
        ],
