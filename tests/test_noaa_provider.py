...
async def test_list_stations(noaa, mock_resilient, make_response):
    mock_resilient([
        make_response({
            "stations": [
                {"id": "8454000", "name": "Providence", "lat": 41.8, "lng": -71.4, "state": "RI"},
                {"id": "8510560", "name": "Montauk", "lat": 41.0, "lng": -71.9, "state": "NY"},
            ]
        }),
    ])
...
async def test_list_stations_max_results(noaa, mock_resilient, make_response):
    mock_resilient([
        make_response({
            "stations": [{"id": str(i), "name": f"S{i}", "lat": 40.0, "lng": -74.0} for i in range(10)]
        }),
    ])
        ...
        make_response({
            "stations": [{
                ...
                "harmonicConstituents": {"HarmonicConstituents": [
                    {"name": "M2", "amplitude": 0.5, "phase_GMT": 123.4, "speed": 28.984},
                ]},
                "floodlevels": {"floodlevels": [{"minor": 1.5, "moderate": 2.0, "major": 2.5}]},
...
async def test_get_extremes(noaa, mock_resilient, make_response):
    mock_resilient([
        make_response({
            "topTenWaterLevels": [
                {"peakDate": "2012-10-29", "height": 2.87, "event": "Sandy", "datum": "MLLW"},
                {"peakDate": "1992-12-11", "height": 2.10, "event": "Nor'easter", "datum": "MLLW"},
            ]
        }),
    ])
...
async def test_get_sea_level_trend(noaa, mock_resilient, make_response):
    ...
    mock_resilient([
        make_response({
            "SeaLvlTrends": [{
                "stationId": "8454000", "stationName": "Providence",
                "trend": 1.378, "trendError": 0.079,
                "startDate": "01/01/1930", "endDate": "12/31/2024",
            }]
        }),
    ])
