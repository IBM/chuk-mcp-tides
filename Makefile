...
# Run the MCP server
run:
	@echo "Running Tides MCP server..."
	@if command -v uv >/dev/null 2>&1; then \
		PYTHONPATH=src uv run python -m chuk_mcp_tides.server; \
	else \
		PYTHONPATH=src python3 -m chuk_mcp_tides.server; \
	fi
...
docker-build:
	...
	docker build -t chuk-mcp-tides:latest .
	@echo "Docker image built: chuk-mcp-tides:latest"

# Run Docker container
docker-run:
	@echo "Running Docker container..."
	docker run --rm -p 8003:8003 chuk-mcp-tides:latest
...
publish-test: build
	...
	echo ""; \
	echo "Install with:"; \
	echo "  pip install --index-url https://test.pypi.org/simple/ chuk-mcp-tides==$$version"
...
publish-manual: build
	...
	echo "Install with: pip install chuk-mcp-tides==$$version"
